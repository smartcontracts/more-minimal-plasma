...
class EventListenerService(BaseServce):
    ...
    Args:
        ...
        confirmations (int): Number of blocks before events should be considered final.
    """
    ...
    def filter_loop(self, event_name):
        ...
        `confirmations` and `confirmations * 2`. This is important because we never want a client to act on an event that isn't
        finalized. We might catch the same event twice, so we hash
        each event and make sure we haven't seen that event yet before broadcasting
        Args:
        ...
        while event_name in self.active_events:
            ...
            event_filter = getattr(self.root_chain.events, event_name).createFilter(
    ...
    def __hash_event(self, event):
        ...
        stringified_event = json.dumps(dict(event), sort_keys=True, cls=HexJsonEncoder)
